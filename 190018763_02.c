...
int tempoDescida = 20, pontua = 4;
int ranq = 0; // se = 1, a partida eh ranqueada
double co_angular = 0;
char p; 
FILE *replay, *config, *arqRank; // os arquivos dos modos de jogo 
...
void despedida(int repete) {
    ...
    // A pontuacao nao eh submetida ao ranking se estiver usando arquivo de replay
    if(ranq && pontuacao != 0 && repete != 2)
        registerMatch();
    if(ranq == 1 && repete == 2)
        printf("So eh possivel ter pontuacao submetida ao ranking se nao estiver usando arquivo de replay!\n");
    if(ranq == 1 && pontuacao == 0)
...
/*  Exibe inicialmente o tabuleiro e gerencia todas as funcoes de jogo 
    Se parametro repete = 1 (cria arquivo de replay)
    Se parametro repete = 2 (usa arquivo de replay)
    Se parametro repete = 3 (nao esta usando ou criando arquivo de replay)
*/
